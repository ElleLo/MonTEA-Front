...
let reducer = (action, state) =>
    switch(action){
        ...
        | ChangeLoadingStatus => ReasonReact.Update({
          ...state,
          sub_loading: !state.sub_loading
        })
    }
        ...
        <div className="p-2">
          ...
          self.state.unsubTags == [||]?
              ...
              : self.state.unsubTags[0][0] == "-1"?
                  <div className="w-full text-center py-4">{str("There are no tags for you to subscribe to.")}</div>
                  :
                  Array.map(item => {
                    incrementId(tableKey);
                      <button
